...
class reinforcement_learning:
    ...
    def state_action_evaluation(self, policy, state_action_value, state_action_returns_num, visit='first'):
        ...
        if visit == 'first':
            ...
            for state_action,G in state_action_return_list:
                state = state_action[0:2]
                action = state_action[2]
                state_action_value[state[0]][state[1]][action] *= state_action_returns_num[state[0]][state[1]][action]
                state_action_returns_num[state[0]][state[1]][action]+=1
                state_action_value[state[0]][state[1]][action] += G
                state_action_value[state[0]][state[1]][action] /= state_action_returns_num[state[0]][state[1]][action]
    ...
    def episode_state_action_return(self, policy, start_state, start_action, episode_len=100):
        ...
        for state, action, reward in state_action_list:
            ...
            if not visit_state_action.__contains__((state,action)):
                visit_state_action.append((state,action))
                state_action_returns.append(([state[0],state[1],action],G))
            # else:
            #     state_action_returns.append(([state[0],state[1],action],0))
