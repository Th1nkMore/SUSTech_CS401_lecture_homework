...
class mdp:
    ...
    def policy_iteration(self, policy_value):
        ## please complete this function by 
        # 1. calculating the action value
        # 2. finding the action index with maximum value
    ...
    def value_iteration(self, threshold=0.01):
        ...
        ## please complete this function by 
        # 1. calculating the policy value 
        # 2. completing the judgement condition indicator delta
            ...
            for i in range (self.state_space[0]):
                for j in range (self.state_space[1]):
                    ...
                    for action_index, action_prob in enumerate(self.policy_matrix[i, j]): 
                        ...
                        # temp_action_value ?
                        if reward + policy_value[next_state_index[0],next_state_index[1]]*state_prob*self.gamma>temp_action_value:
                            temp_action_value = reward + policy_value[next_state_index[0],next_state_index[1]]*state_prob*self.gamma
                    ...
                    # delta = ?
                    if abs(old_value-temp_action_value)>delta:
                        delta = abs(old_value-temp_action_value)
