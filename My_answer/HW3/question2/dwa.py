from re import S
import numpy as np
from math import dist, sqrt, inf
...
class dynamic_window_approach:
    ...
    def cost_function(self, goal_pose, vx, vy, pre_traj, vel_cost_gain=1, goal_cost_gain=1, obstacle_cost_gain=1):
        ...
        # you should complete the function for question2. (Hint: the summary cost of the following functions)
        ...
        return vel_cost_gain*self.vel_cost(vx, vy) + goal_cost_gain*self.cost_to_goal(pre_traj, goal_pose) + obstacle_cost_gain*self.cost_to_obstacle(pre_traj)
    ...
    def cost_to_goal(self, pre_traj, goal):
        ...
        x = pre_traj.pop()
        return sqrt((x[0]-goal[0])**2+(x[1]-goal[1])**2)
    ...
    def cost_to_obstacle(self, pre_traj):
        ...
        return -1*self.point_to_obstalce(np.array([pre_traj[0],pre_traj[0]]))
        # return -1*self.point_to_obstalce(pre_traj[0])
