...
class KalmanFilter:
    def __init__(self, _state, _ip):
        ...
        output = H*next_state + SigmaOutput
        '''
        self.state = _state
        self.ip = _ip
        self.output = None
    ...
    def predict(self, state):
        ...
        self.P += self.SigmaState 
        if self.output is None:
            self.output = self.B @ self.ip + self.SigmaOutput
        else:
            self.output = self.A @ self.output + self.B @ self.ip + self.SigmaOutput
        return self.state

    def update(self, measurement):
        IS = np.dot(self.H, np.dot(self.P, self.H.T)) + self.SigmaState
        Kalman_gain = np.dot(P, np.dot(self.H.T, np.linalg.inv(IS)))
        #gain_factor = 
        self.state = next_state(self.state, self.ip) + np.random.normal([1,1,0], [0.4, 0.4, 0.001], 3)
        self.P = (np.array(np.identity(3)) - Kalman_gain @ self.H) @ self.P
        self.output += Kalman_gain @ (measurement - self.H @ self.output) 
