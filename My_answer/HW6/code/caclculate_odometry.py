...
def calcualte_encoder_shift(row, prev_enc_L, prev_enc_R):
    ...
    deltaL = 0
    deltaR = 0
    if prev_enc_L > 16000 and float(row['posL']) < -16000:
        # forward from + to -
        deltaL = float(row['posL']) - MIN_INT_16
        deltaL = deltaL + (MAX_INT_16 - prev_enc_L)
    elif prev_enc_L < -16000 and float(row['posL']) > 16000:
        # backward from - to +
    ...
    if prev_enc_R > 16000 and float(row['posR']) < -16000:
        # forward from + to -
        deltaR = float(row['posR']) - MIN_INT_16
        deltaR = deltaR + (MAX_INT_16 - prev_enc_R)
    elif prev_enc_R < -16000 and float(row['posR']) > 16000:
        # backward from - to +
...
if __name__ == "__main__":
    ...
    _file = '/home/ljy/IR/intelligent-robot-simulator/Homework/HW6/data/square_left.csv'
